document.addEventListener('DOMContentLoaded', () => {
    ...
    if (bookingForm) {
        ...
        // Seleção flexível do botão para evitar erros se for <button> ou <input>
        const submitBtn = bookingForm.querySelector('button[type="submit"], button, input[type="submit"]');

        // Formatação precisa da data local (YYYY-MM-DD)
        const now = new Date();
        ...
        bookingForm.addEventListener('submit', (e) => {
            ...
            // Se existir botão de submit, aplica animação de carregamento
            if (submitBtn) {
                ...
                // Se não houver botão mapeado, exibe diretamente
                processSuccess();
            }
        });
    }
    ...
    function showSuccessFeedback(form) {
        ...
        // Pequeno atraso garante que a transição CSS de opacidade ocorra perfeitamente
        setTimeout(() => {
